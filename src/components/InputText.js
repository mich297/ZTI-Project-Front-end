...
class InputText extends React.Component {
  ...
  handleChangePass(event) {
    this.setState({
      ...
      progress: !this.props.isLoginOrRegister
        ? this.handleBar(event.target.value)[0]
        : "",
      strength: !this.props.isLoginOrRegister
        ? this.handleBar(event.target.value)[1]
        : "",
    });
  }
  ...
  handlePassCheck(event) {
    if (this.state.valuePass === this.state.valueConfirmPass)
  ...
  showHidePassword() {
    this.setState(
      ...
      () => {
        this.passRef.current.type = this.state.isVisible
          ? "textarea"
          : "password";
        if (this.props.variant === "register")
          this.passRefConfirm.current.type = this.state.isVisible
            ? "textarea"
            : "password";
      }
    );
    ...
    return (
      <>
        ...
        <input
          ...
          onSubmit={this.handlePrevention}
        ></input>
        <input
          type="password"
          ref={this.passRef}
          ...
          onSubmit={this.handlePrevention}
          onBlur={this.handlePassCheck}
        ></input>
        {!isLoginOrRegister && (
          <label for="progressBar" className="progressLabel">
            Password strength:{" "}
            <mark className={this.state.strength}>{this.state.strength}</mark>
          </label>
        )}
        {!isLoginOrRegister && (
          <progress
            ref={this.barRef}
            className={this.state.strength + "Bar"}
            id="progressBar"
        ...
        {!isLoginOrRegister && (
          <input
            type="password"
            id="confirmPass"
            ref={this.passRefConfirm}
            placeholder="confirm password"
            value={this.state.valueConfirmPass}
        ...
        {!isLoginOrRegister && (
          <>
            ...
            ></input>
            <label for="mailField" className="warningLabel">
              {this.state.isMailCorrect === "none"
                ? ""
